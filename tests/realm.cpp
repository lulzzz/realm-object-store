...
TEST_CASE("SharedRealm: compact on launch") {
    ...
    config.automatic_change_notifications = false;
    int num_opens = 0;
    config.should_compact_on_launch_function = [&](__unused size_t total_bytes, __unused size_t used_bytes) {
        num_opens++;
        return num_opens != 2;
    };
    config.schema = Schema{
    ...
    REQUIRE(size_t(File(config.path).get_size()) == 147456); // File size before
    r = Realm::get_shared_realm(config);
    r->close();
    REQUIRE(size_t(File(config.path).get_size()) == 147456); // File size after returning false
    r = Realm::get_shared_realm(config);
    REQUIRE(size_t(File(config.path).get_size()) == 12288); // File size after returning true
