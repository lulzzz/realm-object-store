                      ...
                      bool try_update, Row* out_row)
{
    ...
    if (auto primary_prop = object_schema.primary_key_property()) {
        ...
        auto deref = ctx.deref(primary_value);
        row_index = get_for_primary_key_impl(ctx, *table, *primary_prop, deref);

        if (row_index == realm::not_found) {
            row_index = table->add_empty_row();
            created = true;
            if (ctx.is_null(deref)) {
                ...
                auto value = ctx.to_string(deref);
    ...
    for (size_t i = 0; i < object_schema.persisted_properties.size(); ++i) {
        ...
        if ((!v || ctx.is_null(ctx.deref(v))) && !prop.is_nullable && prop.type != PropertyType::Array) {
            if (!ctx.allow_missing(value))
                throw MissingPropertyValueException(object_schema.name, prop.name);
        }
        if (v)
            object.set_property_value_impl(ctx, prop, ctx.deref(v), try_update);
    }
    return object;
}
